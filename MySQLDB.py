...
class MySQLDB(DatabaseAbsClass):

    def __init__(self):
        self.mydb = None

    def connectUser(self, fileLocation):
        configFile = ET.parse('config.xml').getroot()
        ...
        self.mydb = mysql.connector.connect(host=host, user=user, passwd=passwd, database=database)
        print("Connected")
